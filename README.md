...
- **BigInt policy thresholds must stay within `2^53`.** A CASL condition on a `BigInt` column is evaluated in memory with the row value coerced to a JavaScript number, across read field masking, transactional write verification, and the subscription delete re-check — none of these has an exact SQL fallback (the row-level query filter itself stays exact). `@casl/prisma` cannot express a bigint operand anyway, so a number threshold is the authoring boundary. Past `2^53` a `can` condition over-masks (hides allowed rows and fields) and an inverted `cannot` condition under-masks (leaks).
