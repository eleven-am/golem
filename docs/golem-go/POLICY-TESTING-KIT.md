...
`internal/release`. Separately, `./golemtest` was absent from the public Go API
diff inventory in `internal/compatibility/corpus.go`, which is why that gate
...
`./golemtest` has since been added to the diff inventory as well, so the kit's
own public surface is now inside the frozen corpus.
...
§11 requires, changed the diff inventory in `internal/compatibility/corpus.go`,
which the freeze command and the corpus test both read, and required
regenerating `internal/compatibility/testdata/public-go-api.json`,
`PublicGoAPICorpusSHA256`, `compatibility/manifest.json`, and
`TrustedManifestSHA256`. The change is purely additive; the generated-ABI and
GraphQL corpora are byte-identical.
