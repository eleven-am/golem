...
Status: **complete**. All ten mandatory gates in §9 exist and pass, and every
§10 executable mutant has been observed with a passing baseline and a failing
semantic gate. The checked-social reviewed physical-v2 publication is complete,
and the external generated-application gate passes under `-race` with SQLite and
both mandatory PostgreSQL profiles. The ABI freeze has settled: `./golemtest` is
inside the frozen public Go API corpus, and the kit's gates run in the
repository-wide verification with mandatory PostgreSQL enabled and no skips.
...
**Gate 10 carries one mechanical caveat.** With `GOWORK=off` and no module
proxy, an in-tree consumer requires a `replace` directive. The outcome is
recorded below.
...
**The generated-identity bridges are additive.** §4.1's field-taking signatures
are the only way to reach a `FieldID` and its generation from outside package
`golem`, so §4.3's bridges are required. They widen the kit's public surface
...
way for an in-tree consumer to resolve an unpublished module. The consumer is
otherwise clean: it is a fresh `example.com` module, its schema is generated by
the public CLI, and every Go file in it — handwritten and generated — is proved
to import no `/internal/` package.
