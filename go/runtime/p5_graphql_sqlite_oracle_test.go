...
func newP5OracleHarness(t *testing.T, indexed bool) p5OracleHarness {
	...
	app, err := Open(ctx, withRuntimeTestEvents(t, Config[oraclePrincipal, oracleActor]{Database: p8RuntimeTestDatabase(db, golem.SQLite), Bundle: bundle, Bindings: bindings, Descriptors: descriptors, ResolvePrincipal: func(_ context.Context, p oraclePrincipal) (oracleActor, error) { return p, nil }}))
	if err != nil {
		t.Fatal(err)
	}
...
func newP5RecursiveHarness(t *testing.T) p5RecursiveHarness {
	...
	app, err := Open(ctx, withRuntimeTestEvents(t, Config[oraclePrincipal, oracleActor]{Database: p8RuntimeTestDatabase(database, golem.SQLite), Bundle: bundle, Bindings: bindings, Descriptors: descriptors, ResolvePrincipal: func(_ context.Context, principal oraclePrincipal) (oracleActor, error) { return principal, nil }}))
	if err != nil {
		t.Fatal(err)
	}
	document, err := graphqlschema.Build(compilation)
