...
func newFixture(t testing.TB, userMaxTake, postMaxTake uint32, modes ContractModes, indexedAuthor, exactValues bool) Fixture {
	...
	postBigInt, postDecimal := compilerir.FieldID(id(25)), compilerir.FieldID(id(26))
	...
	model := compilerir.ModelIR{
		...
		Models: []compilerir.ModelDeclIR{
			{ID: user, LogicalName: "User", Table: compilerir.TableBindingIR{PhysicalName: "users"}, Fields: []compilerir.FieldIR{
				...
				relation(userPosts, "Posts", authorship, compilerir.RelationInverse, compilerir.RelationHasMany),
		...
		Relations: []compilerir.RelationIR{{ID: authorship, SourceModel: post, TargetModel: user, SourceField: postAuthor, InverseField: &userPosts, Cardinality: compilerir.RelationMany, LocalFields: []compilerir.FieldID{authorID}, RemoteFields: []compilerir.FieldID{userID}}},
	}
	if exactValues {
	...
	contract := compilerir.ContractIR{FormatVersion: compilerir.ContractFormatVersion, Models: []compilerir.ModelContractIR{
		...
		{ModelID: post, Fields: []compilerir.FieldContractIR{{FieldID: postID, Modes: modes.PostID}, {FieldID: authorID, Modes: modes.AuthorID}, {FieldID: postTitle, Modes: modes.PostTitle}, {FieldID: postAuthor, Modes: modes.PostAuthor}}, Limits: compilerir.LimitContractIR{MaxTake: postMaxTake}},
	}}
	if exactValues {
	...
	sqliteSchema := physicalSchema(compilerir.SQLite, user, post, userID, userName, postID, authorID, postTitle, postBigInt, postDecimal, userKey, postKey, indexedAuthor, exactValues)
	postgresSchema := physicalSchema(compilerir.PostgreSQL, user, post, userID, userName, postID, authorID, postTitle, postBigInt, postDecimal, userKey, postKey, indexedAuthor, exactValues)
...
func physicalSchema(provider compilerir.Provider, user, post compilerir.ModelID, userID, userName, postID, authorID, postTitle, postBigInt, postDecimal compilerir.FieldID, userKey, postKey compilerir.KeyID, indexedAuthor, exactValues bool) physical.PhysicalSchema {
