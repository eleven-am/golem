...
func TestQueryPlanProviderCaptureMutationsAreIsolatedApplicableAndKilled(t *testing.T) {
	mutations := queryPlanProviderCaptureMutations()
	if len(mutations) != 12 {
		t.Fatalf("provider capture mutations = %#v", mutations)
	}
