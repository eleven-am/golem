...
// Retained physical-v3 migration adaptation, pinned by the v3 publication
// provenance test. Equivalence is owned by behavior goldens rather than a
// nonexistent upstream source tag. It is intentionally independent of the
// mutable current planner.
...
// diffHistoricalV3Tagged compares validated normalized physical schemas by stable IDs. It never
// infers a rename from spelling similarity.
func diffHistoricalV3Tagged(before, after physical.PhysicalSchema) (Plan, error) {
	...
	leftFP, err := physical.HistoricalV3PhysicalFingerprint(left)
	if err != nil {
		return Plan{}, err
	}
	rightFP, err := physical.HistoricalV3PhysicalFingerprint(right)
	if err != nil {
		return Plan{}, err
	}
	if reflect.DeepEqual(left, right) {
		return Plan{Provider: right.Provider.Provider, BeforeFingerprint: Digest(leftFP.String()), AfterFingerprint: Digest(rightFP.String())}, nil
	}
	builder := historicalV3DiffBuilder{before: left, after: right, beforeCanonicalVersion: 3, afterCanonicalVersion: 3}
	if err := builder.identifyGeneratedWideningRecreation(); err != nil {
		return Plan{}, err
	}
	beforeSystem, err := physical.HistoricalV3SystemFingerprint(left)
	if err != nil {
		return Plan{}, err
	}
	afterSystem, err := physical.HistoricalV3SystemFingerprint(right)
	if err != nil {
		return Plan{}, err
	}
	initial := false
	if beforeSystem != afterSystem {
		if reflect.DeepEqual(left.System, right.System) {
			// Released v3 provider-runtime transitions changed the physical and
			// system fingerprints without changing a database system object.
		} else if historicalV3EmptySystemSchema(left.System) && len(left.Tables) == 0 && len(left.Extensions) == 0 && len(left.Unmanaged) == 0 && len(right.System.Objects) != 0 {
			initial = true
		} else {
			additions, upgradeErr := historicalV3SystemObjectAdditions(left.System, right.System)
			if upgradeErr != nil {
	...
	record.ID = historicalV3StableOperationID(record.Kind, record.ObjectID, record.Before, record.After)
...
func historicalV3SystemObjectAdditions(before, after physical.SystemSchema) ([]physical.SystemObject, error) {
	...
	if len(additions) != 1 || !historicalV3RegisteredAdditiveSystemObject(additions[0]) {
		return nil, fmt.Errorf("system schema change is not a registered additive system object")
	}
	return additions, nil
}

func historicalV3RegisteredAdditiveSystemObject(object physical.SystemObject) bool {
...
func historicalV3EmptySystemSchema(system physical.SystemSchema) bool {
	return system.Version == 0 && system.Namespace.Name == "" && len(system.Objects) == 0
}

func (b *historicalV3DiffBuilder) extensions() error {
...
type historicalV3DiffBuilder struct {
...
func (b *historicalV3DiffBuilder) identifyGeneratedWideningRecreation() error {
	...
	for _, beforeTable := range b.before.Tables {
		...
		for _, column := range afterTable.Columns {
			...
			if had && !reflect.DeepEqual(before.Storage, column.Storage) {
				...
				if historicalV3PhysicalWideningRepresentation(before.Storage, column.Storage, legacy) {
		...
		for field, beforeColumn := range beforeColumns {
			...
			inputs := historicalV3UniqueFields(historicalV3ExpressionFields(beforeColumn.Generated.Expression))
			if historicalV3FieldsIntersect(inputs, changing) {
		...
		for field, afterColumn := range afterColumns {
			...
			inputs := historicalV3UniqueFields(historicalV3ExpressionFields(afterColumn.Generated.Expression))
			if !historicalV3FieldsIntersect(inputs, changing) {
				continue
			}
			...
			if beforeColumn.Generated.Kind != physical.GeneratedStored || afterColumn.Generated.Kind != physical.GeneratedStored || !historicalV3SameGeneratedColumnAcrossWidening(beforeColumn, afterColumn, b.formatUpgrade) {
			...
			if b.formatUpgrade && beforeColumn.Storage.Kind == physical.StoragePostgreSQLText && afterColumn.Storage.Kind == physical.StoragePostgreSQLVarchar && !historicalV3LegacyVarcharRepresentation(beforeTable, afterTable, field, beforeColumn, afterColumn) {
...
func historicalV3FieldsIntersect(fields []ir.FieldID, selected map[ir.FieldID]bool) bool {
...
func historicalV3UniqueFields(values []ir.FieldID) []ir.FieldID {
...
func historicalV3SameGeneratedColumnAcrossWidening(before, after physical.PhysicalColumn, formatUpgrade bool) bool {
	...
	if !reflect.DeepEqual(left, right) || !historicalV3SameWideningExpression(leftDefault, rightDefault, formatUpgrade) || leftGenerated == nil || rightGenerated == nil || leftGenerated.Kind != rightGenerated.Kind {
		return false
	}
	return historicalV3PhysicalWideningRepresentation(before.Storage, after.Storage, formatUpgrade) && historicalV3SameWideningExpression(&leftGenerated.Expression, &rightGenerated.Expression, formatUpgrade)
}

func historicalV3SameWideningExpression(before, after *physical.Expression, formatUpgrade bool) bool {
	...
	if !reflect.DeepEqual(left, right) || !historicalV3PhysicalWideningRepresentation(leftType, rightType, formatUpgrade) || len(leftOperands) != len(rightOperands) {
		return false
	}
	for index := range leftOperands {
		if !historicalV3SameWideningExpression(&leftOperands[index], &rightOperands[index], formatUpgrade) {
			return false
		}
	}
	return true
...
func historicalV3PhysicalWideningRepresentation(before, after physical.StorageType, formatUpgrade bool) bool {
	if reflect.DeepEqual(before, after) {
		return true
	}
	if historicalV3SafeWidening(ir.PostgreSQL, before, after) {
		return true
	}
...
func (b *historicalV3DiffBuilder) tables() error {
	...
	ids := historicalV3ModelIDs(old, current)
	for _, id := range ids {
		left, had := old[id]
		...
		default:
			if err := historicalV3ValidateConcurrencyTransition(left, right); err != nil {
				return err
			}
...
func historicalV3ValidateConcurrencyTransition(before, after physical.PhysicalTable) error {
...
func (b *historicalV3DiffBuilder) columns(left, right physical.PhysicalTable) error {
	...
	ids := historicalV3FieldIDs(old, cur)
	for _, id := range ids {
		a, had := old[id]
		z, has := cur[id]
		switch {
		...
		default:
			...
			if !reflect.DeepEqual(a.Storage, z.Storage) {
				...
				if b.formatUpgrade && a.Storage.Kind == physical.StoragePostgreSQLText && z.Storage.Kind == physical.StoragePostgreSQLVarchar {
					if !historicalV3LegacyVarcharRepresentation(left, right, id, a, z) {
					...
					risk = RiskRewrite
				} else if historicalV3SafeWidening(b.after.Provider.Provider, a.Storage, z.Storage) {
					risk = RiskRewrite
				}
...
func (b *historicalV3DiffBuilder) requiredConcurrencyColumn(column physical.PhysicalColumn) error {
	id := string(column.ID)
...
func (b *historicalV3DiffBuilder) requiredColumnBackfill(column physical.PhysicalColumn) error {
	id := string(column.ID)
...
func historicalV3LegacyVarcharRepresentation(beforeTable, afterTable physical.PhysicalTable, field ir.FieldID, before, after physical.PhysicalColumn) bool {
	...
	if !historicalV3SameColumnAcrossV1V2Representation(before, after) {
		return false
	}
...
func historicalV3SameColumnAcrossV1V2Representation(before, after physical.PhysicalColumn) bool {
	left, right := before, after
	left.Storage, right.Storage = physical.StorageType{}, physical.StorageType{}
	leftDefault, rightDefault := left.Default.Expression, right.Default.Expression
	left.Default.Expression, right.Default.Expression = nil, nil
	leftGenerated, rightGenerated := left.Generated, right.Generated
	left.Generated, right.Generated = nil, nil
	if !reflect.DeepEqual(left, right) || !historicalV3SameRepresentationExpression(leftDefault, rightDefault) {
		return false
	}
	if leftGenerated == nil || rightGenerated == nil {
		return leftGenerated == nil && rightGenerated == nil
	}
	return leftGenerated.Kind == rightGenerated.Kind && historicalV3SameRepresentationExpression(&leftGenerated.Expression, &rightGenerated.Expression)
}

func historicalV3SameRepresentationExpression(before, after *physical.Expression) bool {
	if before == nil || after == nil {
		return before == nil && after == nil
	}
	left, right := *before, *after
	...
	leftType, rightType := left.Type, right.Type
	left.Type, right.Type = physical.StorageType{}, physical.StorageType{}
	if !reflect.DeepEqual(left, right) || !historicalV3SameRepresentationStorage(leftType, rightType) || len(leftOperands) != len(rightOperands) {
		return false
	}
	for index := range leftOperands {
		if !historicalV3SameRepresentationExpression(&leftOperands[index], &rightOperands[index]) {
			return false
		}
	}
	return true
...
func historicalV3SameRepresentationStorage(before, after physical.StorageType) bool {
...
func (b *historicalV3DiffBuilder) objects(left, right physical.PhysicalTable) error {
	if err := b.key(left.ID, left.PrimaryKey, right.PrimaryKey, AddPrimaryKey, DropPrimaryKey, RiskDataLoss); err != nil {
		return err
	}
	if err := historicalV3DiffByID(left.Uniques, right.Uniques, func(v physical.PhysicalKey) string { return string(v.ID) }, func(v any, add bool) error {
		if add {
			return b.add(AddUnique, 40, historicalV3ObjectID(v), nil, v, RiskDataLoss)
		}
		return b.add(DropUnique, 60, historicalV3ObjectID(v), v, nil, RiskSafe)
	}, b); err != nil {
		return err
	}
	if err := historicalV3DiffByID(left.ForeignKeys, right.ForeignKeys, func(v physical.PhysicalForeignKey) string { return string(v.ID) }, func(v any, add bool) error {
		if add {
			return b.add(AddForeignKey, 50, historicalV3ObjectID(v), nil, v, RiskLocking)
		}
		return b.add(DropForeignKey, 60, historicalV3ObjectID(v), v, nil, RiskSafe)
	}, b); err != nil {
		return err
	}
	if err := historicalV3DiffByID(left.Checks, right.Checks, func(v physical.PhysicalCheck) string { return string(v.ID) }, func(v any, add bool) error {
		if add {
			return b.add(AddCheck, 40, historicalV3ObjectID(v), nil, v, RiskDataLoss)
		}
		return b.add(DropCheck, 60, historicalV3ObjectID(v), v, nil, RiskSafe)
	}, b); err != nil {
		return err
	}
...
func (b *historicalV3DiffBuilder) indexes(before, after []physical.PhysicalIndex) error {
...
func (b *historicalV3DiffBuilder) recreateDestructiveDependents() error {
...
func (b *historicalV3DiffBuilder) recreateTableDependents(before, after physical.PhysicalTable, field ir.FieldID) error {
	if before.PrimaryKey != nil && historicalV3ObjectUsesField(before, *before.PrimaryKey, field) {
	...
	for _, value := range before.Uniques {
		if !historicalV3ObjectUsesField(before, value, field) {
			continue
		}
		if err := b.add(DropUnique, 60, string(value.ID), value, nil, RiskSafe); err != nil {
			return err
		}
		if restored, ok := historicalV3FindKey(after.Uniques, value.ID); ok {
	...
	for _, value := range before.ForeignKeys {
		...
		if restored, ok := historicalV3FindForeignKey(after.ForeignKeys, value.ID); ok {
	...
	for _, value := range before.Checks {
		...
		if restored, ok := historicalV3FindCheck(after.Checks, value.ID); ok {
	...
	for _, value := range before.Indexes {
		...
		if restored, ok := historicalV3FindIndex(after.Indexes, value.ID); ok {
...
func (b *historicalV3DiffBuilder) key(table ir.ModelID, a, z *physical.PhysicalKey, add, drop OperationKind, risk Risk) error {
...
func (b *historicalV3DiffBuilder) add(kind OperationKind, stage uint16, id string, before, after any, risk Risk) error {
	a, err := historicalV3FragmentVersion(before, b.beforeCanonicalVersion)
	if err != nil {
		return err
	}
	z, err := historicalV3FragmentVersion(after, b.afterCanonicalVersion)
	if err != nil {
		return err
	}
	op := Operation{Kind: kind, Stage: stage, ObjectID: id, Before: a, After: z, Mode: Transactional, Risk: risk, LogicalPath: id}
	op.ID = historicalV3StableOperationID(kind, id, a, z)
...
func (b *historicalV3DiffBuilder) dependencies() {
	...
	for index := range b.operations {
		...
		if !b.historicalReplay && op.Kind == AddColumn {
			if column := afterColumns[ir.FieldID(op.ObjectID)]; column.Generated != nil {
				for _, field := range historicalV3ExpressionFields(column.Generated.Expression) {
		...
		if op.Kind == AddPrimaryKey || op.Kind == AddUnique || op.Kind == AddForeignKey || op.Kind == AddCheck || op.Kind == CreateIndex {
			if table, ok := tables[tableID]; ok {
				for _, field := range historicalV3ReferencedFields(table, op.ObjectID) {
					addDep(op, AddColumn, string(field))
		...
		if op.Kind == DropPrimaryKey || op.Kind == DropUnique {
			if owner, ok := tables[tableID]; ok {
				columns := historicalV3ReferencedFields(owner, op.ObjectID)
				for _, table := range tables {
					for _, fk := range table.ForeignKeys {
		...
		if op.Kind == DropColumn || op.Kind == AlterColumnType || op.Kind == AlterColumnNullability {
			for _, table := range append(append([]physical.PhysicalTable(nil), b.before.Tables...), b.after.Tables...) {
				for _, object := range historicalV3DependentObjects(table, ir.FieldID(op.ObjectID)) {
		...
		if !b.historicalReplay && op.Kind == DropColumn {
			...
			for generated, column := range beforeColumns {
				if column.Generated != nil && historicalV3ContainsField(historicalV3ExpressionFields(column.Generated.Expression), field) {
			...
			case DropColumn:
				...
				if tableID, generated := b.generatedDrops[field]; generated {
					for dependent, owner := range b.generatedDrops {
						if owner == tableID && historicalV3ContainsField(b.generatedInputs[dependent], field) {
...
func historicalV3ReferencedFields(table physical.PhysicalTable, id string) []ir.FieldID {
		...
		if string(v.ID) == id {
			...
			for _, key := range v.Keys {
				...
				if key.Expression != nil {
					out = append(out, historicalV3ExpressionFields(*key.Expression)...)
				}
			}
			out = append(out, v.Include...)
			if v.Predicate != nil {
				out = append(out, historicalV3ExpressionFields(*v.Predicate)...)
			}
			return out
		}
	}
	for _, v := range table.Checks {
		if string(v.ID) == id {
			return historicalV3ExpressionFields(v.Expression)
		}
	}
	return nil
}
func historicalV3DependentObjects(table physical.PhysicalTable, field ir.FieldID) []string {
	var out []string
	if table.PrimaryKey != nil && historicalV3ContainsField(table.PrimaryKey.Columns, field) {
		out = append(out, string(table.PrimaryKey.ID))
	}
	for _, v := range table.Uniques {
		if historicalV3ContainsField(v.Columns, field) {
			out = append(out, string(v.ID))
		}
	}
	for _, v := range table.ForeignKeys {
		if historicalV3ContainsField(v.Columns, field) || historicalV3ContainsField(v.ReferencedColumns, field) {
			out = append(out, string(v.ID))
		}
	}
	for _, v := range table.Indexes {
		if historicalV3ContainsField(v.Include, field) {
			out = append(out, string(v.ID))
			continue
		}
		for _, key := range v.Keys {
			if (key.Column != nil && *key.Column == field) || (key.Expression != nil && historicalV3ContainsField(historicalV3ExpressionFields(*key.Expression), field)) {
				out = append(out, string(v.ID))
				break
			}
		}
		if v.Predicate != nil && historicalV3ContainsField(historicalV3ExpressionFields(*v.Predicate), field) {
			out = append(out, string(v.ID))
		}
	}
	for _, v := range table.Checks {
		if historicalV3ContainsField(historicalV3ExpressionFields(v.Expression), field) {
			out = append(out, string(v.ID))
		}
	}
	return out
...
func historicalV3ExpressionFields(expression physical.Expression) []ir.FieldID {
	var out []ir.FieldID
	if expression.Column != nil {
		out = append(out, *expression.Column)
	}
	for _, operand := range expression.Operands {
		out = append(out, historicalV3ExpressionFields(operand)...)
	}
	return out
}

func historicalV3ObjectUsesField(_ physical.PhysicalTable, object any, field ir.FieldID) bool {
	switch value := object.(type) {
	case physical.PhysicalKey:
		return historicalV3ContainsField(value.Columns, field)
	case physical.PhysicalForeignKey:
		return historicalV3ContainsField(value.Columns, field) || historicalV3ContainsField(value.ReferencedColumns, field)
	case physical.PhysicalCheck:
		return historicalV3ContainsField(historicalV3ExpressionFields(value.Expression), field)
	case physical.PhysicalIndex:
		if historicalV3ContainsField(value.Include, field) {
			return true
		}
		if value.Predicate != nil && historicalV3ContainsField(historicalV3ExpressionFields(*value.Predicate), field) {
			return true
		}
		for _, key := range value.Keys {
			...
			if key.Expression != nil && historicalV3ContainsField(historicalV3ExpressionFields(*key.Expression), field) {
				return true
			}
		}
	}
...
func historicalV3FindKey(values []physical.PhysicalKey, id ir.KeyID) (physical.PhysicalKey, bool) {
	for _, value := range values {
		if value.ID == id {
			return value, true
		}
...
func historicalV3FindForeignKey(values []physical.PhysicalForeignKey, id ir.ForeignKeyID) (physical.PhysicalForeignKey, bool) {
	for _, value := range values {
		if value.ID == id {
			return value, true
		}
...
func historicalV3FindCheck(values []physical.PhysicalCheck, id ir.CheckID) (physical.PhysicalCheck, bool) {
	for _, value := range values {
		if value.ID == id {
			return value, true
		}
...
func historicalV3FindIndex(values []physical.PhysicalIndex, id ir.IndexID) (physical.PhysicalIndex, bool) {
	for _, value := range values {
		if value.ID == id {
			return value, true
		}
...
func historicalV3ContainsField(values []ir.FieldID, field ir.FieldID) bool {
	for _, value := range values {
		if value == field {
			return true
		}
...
func historicalV3Fragment(value any) (Digest, error) {
	return historicalV3FragmentVersion(value, 3)
}

func historicalV3FragmentVersion(value any, version uint32) (Digest, error) {
	if value == nil {
		return "", nil
	}
	if version != 3 {
	...
	encoded, err := physical.HistoricalV3CanonicalFragment(value)
	if err != nil {
		return "", err
	}
...
func historicalV3StableOperationID(kind OperationKind, id string, before, after Digest) OperationID {
	sum := sha256.Sum256([]byte(string(kind) + "\x00" + id + "\x00" + string(before) + "\x00" + string(after)))
	return OperationID(hex.EncodeToString(sum[:16]))
}

func historicalV3DiffByID[T any](a, z []T, id func(T) string, emit func(any, bool) error, _ *historicalV3DiffBuilder) error {
	old := map[string]T{}
	cur := map[string]T{}
	for _, v := range a {
		old[id(v)] = v
	...
	ids := historicalV3StringIDs(old, cur)
	for _, key := range ids {
		left, had := old[key]
		right, has := cur[key]
...
func historicalV3ObjectID(v any) string {
	switch x := v.(type) {
	case physical.PhysicalKey:
		return string(x.ID)
	case physical.PhysicalForeignKey:
...
func historicalV3ModelIDs[A, B any](a map[ir.ModelID]A, b map[ir.ModelID]B) []ir.ModelID {
...
func historicalV3FieldIDs[A, B any](a map[ir.FieldID]A, b map[ir.FieldID]B) []ir.FieldID {
	seen := map[ir.FieldID]bool{}
	var out []ir.FieldID
	for id := range a {
		seen[id] = true
...
func historicalV3StringIDs[T any](a, b map[string]T) []string {
...
func historicalV3SafeWidening(provider ir.Provider, before, after physical.StorageType) bool {
	...
	case physical.StoragePostgreSQLSmallInt:
		return historicalV3UnparameterizedPair(before, after) && (after.Kind == physical.StoragePostgreSQLInteger || after.Kind == physical.StoragePostgreSQLBigInt)
	case physical.StoragePostgreSQLInteger:
		return historicalV3UnparameterizedPair(before, after) && after.Kind == physical.StoragePostgreSQLBigInt
	case physical.StoragePostgreSQLReal:
		return historicalV3UnparameterizedPair(before, after) && after.Kind == physical.StoragePostgreSQLDouble
	...
	case physical.StoragePostgreSQLTime, physical.StoragePostgreSQLTimestampTZ:
		...
		return after.Length >= before.Length && after.Length <= historicalV3TemporalPrecision
	default:
		return false
	}
}

func historicalV3UnparameterizedPair(values ...physical.StorageType) bool {
	for _, value := range values {
