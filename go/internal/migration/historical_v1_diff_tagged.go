...
// Retained verbatim-algebra adaptation of go/v0.0.1 diff.go.
// Tagged source SHA-256: 4d7271550104a57a6f9766bbe9456a5544cdf429eda4540df366550ace572679.
...
const (
	historicalV1DiffUpstreamSHA256 = "4d7271550104a57a6f9766bbe9456a5544cdf429eda4540df366550ace572679"
	historicalV1DiffUpstreamLines  = 871
)

// diffHistoricalV1Tagged compares validated normalized physical schemas by stable IDs. It never
// infers a rename from spelling similarity.
func diffHistoricalV1Tagged(before, after physical.PhysicalSchema) (Plan, error) {
	...
	builder := historicalV1DiffBuilder{before: left, after: right}
	...
	if beforeSystem != afterSystem {
		if reflect.DeepEqual(left.System, right.System) {
			// Released v1 provider-runtime transitions changed the physical and
			// system fingerprints without changing a database system object.
		} else if historicalV1EmptySystemSchema(left.System) && len(left.Tables) == 0 && len(left.Extensions) == 0 && len(left.Unmanaged) == 0 && len(right.System.Objects) != 0 {
			initial = true
		} else {
			additions, upgradeErr := historicalV1SystemObjectAdditions(left.System, right.System)
			if upgradeErr != nil {
	...
	record.ID = historicalV1StableOperationID(record.Kind, record.ObjectID, record.Before, record.After)
...
func historicalV1SystemObjectAdditions(before, after physical.SystemSchema) ([]physical.SystemObject, error) {
	...
	if len(additions) != 1 || !historicalV1RegisteredAdditiveSystemObject(additions[0]) {
		return nil, fmt.Errorf("system schema change is not a registered additive system object")
	}
	return additions, nil
}

func historicalV1RegisteredAdditiveSystemObject(object physical.SystemObject) bool {
...
func historicalV1EmptySystemSchema(system physical.SystemSchema) bool {
	return system.Version == 0 && system.Namespace.Name == "" && len(system.Objects) == 0
}

type historicalV1DiffBuilder struct {
	before, after physical.PhysicalSchema
	operations    []Operation
}

func (b *historicalV1DiffBuilder) tables() error {
	...
	ids := historicalV1ModelIDs(old, current)
	for _, id := range ids {
		left, had := old[id]
...
func (b *historicalV1DiffBuilder) columns(left, right physical.PhysicalTable) error {
	...
	ids := historicalV1FieldIDs(old, cur)
	for _, id := range ids {
		a, had := old[id]
		z, has := cur[id]
		switch {
...
func (b *historicalV1DiffBuilder) objects(left, right physical.PhysicalTable) error {
	if err := b.key(left.ID, left.PrimaryKey, right.PrimaryKey, AddPrimaryKey, DropPrimaryKey, RiskDataLoss); err != nil {
		return err
	}
	if err := historicalV1DiffByID(left.Uniques, right.Uniques, func(v physical.PhysicalKey) string { return string(v.ID) }, func(v any, add bool) error {
		if add {
			return b.add(AddUnique, 40, historicalV1ObjectID(v), nil, v, RiskDataLoss)
		}
		return b.add(DropUnique, 60, historicalV1ObjectID(v), v, nil, RiskSafe)
	}, b); err != nil {
		return err
	}
	if err := historicalV1DiffByID(left.ForeignKeys, right.ForeignKeys, func(v physical.PhysicalForeignKey) string { return string(v.ID) }, func(v any, add bool) error {
		if add {
			return b.add(AddForeignKey, 50, historicalV1ObjectID(v), nil, v, RiskLocking)
		}
		return b.add(DropForeignKey, 60, historicalV1ObjectID(v), v, nil, RiskSafe)
	}, b); err != nil {
		return err
	}
	if err := historicalV1DiffByID(left.Checks, right.Checks, func(v physical.PhysicalCheck) string { return string(v.ID) }, func(v any, add bool) error {
		if add {
			return b.add(AddCheck, 40, historicalV1ObjectID(v), nil, v, RiskDataLoss)
		}
		return b.add(DropCheck, 60, historicalV1ObjectID(v), v, nil, RiskSafe)
	}, b); err != nil {
		return err
	}
...
func (b *historicalV1DiffBuilder) indexes(before, after []physical.PhysicalIndex) error {
...
func (b *historicalV1DiffBuilder) recreateDestructiveDependents() error {
...
func (b *historicalV1DiffBuilder) recreateTableDependents(before, after physical.PhysicalTable, field ir.FieldID) error {
	if before.PrimaryKey != nil && historicalV1ObjectUsesField(before, *before.PrimaryKey, field) {
	...
	for _, value := range before.Uniques {
		if !historicalV1ObjectUsesField(before, value, field) {
			continue
		}
		if err := b.add(DropUnique, 60, string(value.ID), value, nil, RiskSafe); err != nil {
			return err
		}
		if restored, ok := historicalV1FindKey(after.Uniques, value.ID); ok {
	...
	for _, value := range before.ForeignKeys {
		...
		if restored, ok := historicalV1FindForeignKey(after.ForeignKeys, value.ID); ok {
	...
	for _, value := range before.Checks {
		...
		if restored, ok := historicalV1FindCheck(after.Checks, value.ID); ok {
	...
	for _, value := range before.Indexes {
		...
		if restored, ok := historicalV1FindIndex(after.Indexes, value.ID); ok {
...
func (b *historicalV1DiffBuilder) key(table ir.ModelID, a, z *physical.PhysicalKey, add, drop OperationKind, risk Risk) error {
...
func (b *historicalV1DiffBuilder) add(kind OperationKind, stage uint16, id string, before, after any, risk Risk) error {
	a, err := historicalV1Fragment(before)
	if err != nil {
		return err
	}
	z, err := historicalV1Fragment(after)
	if err != nil {
		return err
	}
	op := Operation{Kind: kind, Stage: stage, ObjectID: id, Before: a, After: z, Mode: Transactional, Risk: risk, LogicalPath: id}
	op.ID = historicalV1StableOperationID(kind, id, a, z)
...
func (b *historicalV1DiffBuilder) dependencies() {
	...
	for index := range b.operations {
		...
		if op.Kind == AddPrimaryKey || op.Kind == AddUnique || op.Kind == AddForeignKey || op.Kind == AddCheck || op.Kind == CreateIndex {
			if table, ok := tables[tableID]; ok {
				for _, field := range historicalV1ReferencedFields(table, op.ObjectID) {
		...
		if op.Kind == DropColumn || op.Kind == AlterColumnType || op.Kind == AlterColumnNullability {
			for _, table := range tables {
				for _, object := range historicalV1DependentObjects(table, ir.FieldID(op.ObjectID)) {
...
func historicalV1ReferencedFields(table physical.PhysicalTable, id string) []ir.FieldID {
		...
		if string(v.ID) == id {
			...
			for _, key := range v.Keys {
				...
				if key.Expression != nil {
					out = append(out, historicalV1ExpressionFields(*key.Expression)...)
				}
			}
			out = append(out, v.Include...)
			if v.Predicate != nil {
				out = append(out, historicalV1ExpressionFields(*v.Predicate)...)
			}
			return out
		}
	}
	for _, v := range table.Checks {
		if string(v.ID) == id {
			return historicalV1ExpressionFields(v.Expression)
		}
	}
	return nil
}
func historicalV1DependentObjects(table physical.PhysicalTable, field ir.FieldID) []string {
	var out []string
	if table.PrimaryKey != nil && historicalV1ContainsField(table.PrimaryKey.Columns, field) {
		out = append(out, string(table.PrimaryKey.ID))
	}
	for _, v := range table.Uniques {
		if historicalV1ContainsField(v.Columns, field) {
			out = append(out, string(v.ID))
		}
	}
	for _, v := range table.ForeignKeys {
		if historicalV1ContainsField(v.Columns, field) || historicalV1ContainsField(v.ReferencedColumns, field) {
			out = append(out, string(v.ID))
		}
	}
	for _, v := range table.Indexes {
		if historicalV1ContainsField(v.Include, field) {
			out = append(out, string(v.ID))
			continue
		}
		for _, key := range v.Keys {
			if (key.Column != nil && *key.Column == field) || (key.Expression != nil && historicalV1ContainsField(historicalV1ExpressionFields(*key.Expression), field)) {
				out = append(out, string(v.ID))
				break
			}
		}
		if v.Predicate != nil && historicalV1ContainsField(historicalV1ExpressionFields(*v.Predicate), field) {
			out = append(out, string(v.ID))
		}
	}
	for _, v := range table.Checks {
		if historicalV1ContainsField(historicalV1ExpressionFields(v.Expression), field) {
			out = append(out, string(v.ID))
		}
	}
	return out
...
func historicalV1ExpressionFields(expression physical.Expression) []ir.FieldID {
	var out []ir.FieldID
	if expression.Column != nil {
		out = append(out, *expression.Column)
	}
	for _, operand := range expression.Operands {
		out = append(out, historicalV1ExpressionFields(operand)...)
	}
	return out
}

func historicalV1ObjectUsesField(_ physical.PhysicalTable, object any, field ir.FieldID) bool {
	switch value := object.(type) {
	case physical.PhysicalKey:
		return historicalV1ContainsField(value.Columns, field)
	case physical.PhysicalForeignKey:
		return historicalV1ContainsField(value.Columns, field) || historicalV1ContainsField(value.ReferencedColumns, field)
	case physical.PhysicalCheck:
		return historicalV1ContainsField(historicalV1ExpressionFields(value.Expression), field)
	case physical.PhysicalIndex:
		if historicalV1ContainsField(value.Include, field) {
			return true
		}
		if value.Predicate != nil && historicalV1ContainsField(historicalV1ExpressionFields(*value.Predicate), field) {
			return true
		}
		for _, key := range value.Keys {
			...
			if key.Expression != nil && historicalV1ContainsField(historicalV1ExpressionFields(*key.Expression), field) {
				return true
			}
		}
	}
...
func historicalV1FindKey(values []physical.PhysicalKey, id ir.KeyID) (physical.PhysicalKey, bool) {
...
func historicalV1FindForeignKey(values []physical.PhysicalForeignKey, id ir.ForeignKeyID) (physical.PhysicalForeignKey, bool) {
	for _, value := range values {
		if value.ID == id {
			return value, true
		}
...
func historicalV1FindCheck(values []physical.PhysicalCheck, id ir.CheckID) (physical.PhysicalCheck, bool) {
	for _, value := range values {
		if value.ID == id {
			return value, true
		}
...
func historicalV1FindIndex(values []physical.PhysicalIndex, id ir.IndexID) (physical.PhysicalIndex, bool) {
	for _, value := range values {
		if value.ID == id {
			return value, true
		}
...
func historicalV1ContainsField(values []ir.FieldID, field ir.FieldID) bool {
	for _, value := range values {
		if value == field {
			return true
		}
...
func historicalV1Fragment(value any) (Digest, error) {
	if value == nil {
		return "", nil
	}
...
func historicalV1StableOperationID(kind OperationKind, id string, before, after Digest) OperationID {
	sum := sha256.Sum256([]byte(string(kind) + "\x00" + id + "\x00" + string(before) + "\x00" + string(after)))
	return OperationID(hex.EncodeToString(sum[:16]))
}

func historicalV1DiffByID[T any](a, z []T, id func(T) string, emit func(any, bool) error, _ *historicalV1DiffBuilder) error {
	old := map[string]T{}
	cur := map[string]T{}
	for _, v := range a {
		old[id(v)] = v
	...
	ids := historicalV1StringIDs(old, cur)
	for _, key := range ids {
		left, had := old[key]
		right, has := cur[key]
...
func historicalV1ObjectID(v any) string {
	switch x := v.(type) {
	case physical.PhysicalKey:
		return string(x.ID)
	case physical.PhysicalForeignKey:
...
func historicalV1ModelIDs[A, B any](a map[ir.ModelID]A, b map[ir.ModelID]B) []ir.ModelID {
...
func historicalV1FieldIDs[A, B any](a map[ir.FieldID]A, b map[ir.FieldID]B) []ir.FieldID {
...
func historicalV1StringIDs[T any](a, b map[string]T) []string {
