...
func emitRuntimeSurface(source *bytes.Buffer, actorType, contextAlias, fmtAlias, providerAlias, embeddingAlias, queueAlias, golemAlias, runtimeAlias, queryplanAlias, eventsAlias, observeAlias string, models []ir.ModelDeclIR, aliases map[string]string, contracts map[ir.ModelID]ir.ModelContractIR, semantic map[ir.ModelID][]semanticcontract.Index) {
	...
	source.WriteString("// system clients for the caller transaction itself, so their reads and\n")
	source.WriteString("// writes run inside that transaction and commit or roll back with every\n")
	source.WriteString("// caller write in it.\n")
	source.WriteString("//\n")
	source.WriteString("// Nothing performed through them is policy-checked. No caller rule, field\n")
	source.WriteString("// restriction, or row predicate applies, exactly as if the write had been\n")
	source.WriteString("// made by App.System. Use it only for a value the application owns and no\n")
	source.WriteString("// client may set, and keep the call at the site that needs it: every call\n")
	source.WriteString("// is observed as transaction.system_escape.\n")
	source.WriteString("func SystemEscape[P any](transaction *CallerTx[P]) *SystemTx[P] {\n")
	fmt.Fprintf(source, "\tvar inner *%s.SystemTx[P, %s]\n", runtimeAlias, actorType)
	fmt.Fprintf(source, "\tif transaction != nil { inner = %s.CallerTxSystem(transaction.runtime) }\n", runtimeAlias)
