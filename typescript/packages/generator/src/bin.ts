...
generatorHandler({
  ...
  onGenerate: async (options) => {
    ...
    const publicModelNames = options.dmmf.datamodel.models
      .map((model) => model.name)
      .filter((name) => name !== 'GolemUpsertGuard');
    ...
    await writeFile(
      ...
      emitClientModule(publicModelNames, clientImport),
    );
    await writeFile(
      join(output, 'types.ts'),
      emitTypesModule(publicModelNames, clientImport),
    );
  },
});
